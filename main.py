...
ROLE_TO_ADD_2 = 1434608888145248257  # رول إضافي
ROLE_TO_REMOVE = 1461747562808606884  # الرول المأخوذ
# -----------------

ROLE_TO_ADD_3 = 1467641757385883841  # رول القرد 🐒
ROLE_TO_ADD_4 = 1434608888145248257  # رول إضافي
ROLE_TO_ADD_5 = 1467641677199184227  # رول إضافي
...
async def on_message(message):
    ...
    if message.content.startswith('!استعادة'):
        ...
        if not message.mentions:
            ...
            return
        guild = message.guild
        r_a1, r_a2,r_a3, r_t = guild.get_role(ROLE_TO_ADD_3), guild.get_role(ROLE_TO_ADD_4),guild.get_role(ROLE_TO_ADD_5), guild.get_role(ROLE_TO_REMOVE1)
        for member in message.mentions:
            try:
                if r_a1: await member.remove_roles(r_a1)
                if r_a2: await member.remove_roles(r_a2)
                if r_a3: await member.remove_roles(r_a3)
